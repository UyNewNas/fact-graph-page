...
function flowData(raw) {
  const nodes = levels(safeArray(raw.nodes).filter(n => n && n.id).map(n => ({ id:n.id, title:clean(n.title)||'Untitled fact', author:clean(n.author)||'unknown', assistant:clean(n.assistant), preview:clean(n.statement_preview), revoked:!!n.revoked, predecessors:safeArray(n.predecessors), children:safeArray(n.children) })))
...
function Inspector({ node, facts, isStatic, onClose, onJump }) {
  ...
  const fullText=JSON.stringify({id:node.id,title:node.title,author:node.author,assistant:node.assistant,statement_preview:node.preview,revoked:node.revoked,status:node.revoked?'REVOKED':'VERIFIED',type:kind(node).toUpperCase(),level:node.level,predecessors:node.predecessors,children:node.children,detail},null,2)
...
function Canvas() {
  const [graph,setGraph]=useState(null),[selected,setSelected]=useState(null),[error,setError]=useState(''),[flowReady,setFlowReady]=useState(false),[activeLevel,setActiveLevel]=useState(0); const [searchId,setSearchId]=useState(''),[searchError,setSearchError]=useState(''); const [dark,setDark]=useState(()=>{try{const saved=localStorage.getItem('fact-graph-theme');return saved?saved==='dark':window.matchMedia('(prefers-color-scheme: dark)').matches}catch{return false}}); const canvasRef=useRef(null); const { fitView, setCenter, getZoom }=useReactFlow()
  ...
  const renderedNodes=useMemo(()=>graph?graph.nodes.map(n=>({...n,selected:n.id===selected?.id})):[],[graph,selected])
  ...
  useEffect(()=>{document.documentElement.dataset.theme=dark?'dark':'light';try{localStorage.setItem('fact-graph-theme',dark?'dark':'light')}catch{}},[dark])
  ...
  const locateId=useCallback(e=>{e?.preventDefault();const query=searchId.trim().toLowerCase();if(!query){setSearchError('请输入 Fact ID');return}const exact=graph?.nodes.find(n=>n.id.toLowerCase()===query);const matches=exact?[exact]:graph?.nodes.filter(n=>n.id.toLowerCase().startsWith(query))||[];if(matches.length===1){setSearchError('');jump(matches[0].id)}else setSearchError(matches.length?`匹配 ${matches.length} 个，请继续输入`:'未找到该 Fact ID')},[graph,searchId,jump])
  ...
  return <div className="shell"><header className="bar"><div className="brand"><i></i><b>Fact Graph</b><span>{graph.project}</span></div><form className="id-search" onSubmit={locateId}><input value={searchId} onChange={e=>{setSearchId(e.target.value);setSearchError('')}} placeholder="搜索 Fact ID" aria-label="搜索 Fact ID"/><button type="submit">定位</button>{searchError&&<span role="status">{searchError}</span>}</form><div className="search">只读 Dify 风格工作流画布 · {isStatic?'静态快照':'实时本地数据'}</div><button className="theme-toggle" onClick={()=>setDark(value=>!value)} aria-pressed={dark}>{dark?'日间模式':'夜间模式'}</button><button onClick={load}>重新载入</button></header><main className="workspace"><Inspector node={selected} facts={graph.factDetails} isStatic={isStatic} onClose={()=>setSelected(null)} onJump={jump}/><div className="canvas" ref={canvasRef}><ReactFlow nodes={renderedNodes} edges={graph.edges} nodeTypes={nodeTypes} fitView fitViewOptions={{padding:.12,maxZoom:.72}} onInit={()=>setFlowReady(true)} onMoveEnd={syncLevel} nodesDraggable={false} onNodeClick={(_,n)=>jump(n.id)} proOptions={{hideAttribution:true}}><Background gap={16} size={1} color={dark?'#243248':'#dfe6ee'}/><Controls showInteractive={false}/><MiniMap nodeColor={n=>n.data?.revoked?'#d95563':!n.data?.predecessors?.length?'#21a66c':'#526cff'} nodeStrokeColor={dark?'#111b2c':'#ffffff'} nodeStrokeWidth={5} nodeBorderRadius={8} bgColor={dark?'#111b2c':'#ffffff'} maskColor={dark?'rgba(116,139,255,.18)':'rgba(82,108,255,.10)'} maskStrokeColor={dark?'#8b9cff':'#526cff'} maskStrokeWidth={2} zoomable pannable/><LevelNavigator level={activeLevel} max={maxLevel} onChange={goLevel}/><Panel position="top-left" className="overview"><b>{graph.facts.length}</b><span>已验证事实</span><b>{graph.edges.length}</b><span>依赖边</span></Panel></ReactFlow></div></main></div>
}
