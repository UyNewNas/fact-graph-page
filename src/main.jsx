...
function Inspector({ node, facts, onClose, onJump }) {
  const [copyState,setCopyState]=useState('')
  if (!node) return null
  const detail=facts[node.id]; const refs=safeArray(detail?.external_refs); const glossary=detail?.glossary&&typeof detail.glossary==='object'?detail.glossary:{}
...
function Canvas() {
  ...
  const load=useCallback(async()=>{try{setError('');const r=await fetch(`${import.meta.env.BASE_URL}fact-graph.json`,{cache:'no-store'});if(!r.ok)throw Error('未找到静态快照（请先运行 npm run snapshot）');setGraph(flowData(await r.json()))}catch(e){setError(e.message)}},[])
